...
def cut_roi(self,image, roi):
    w = self.pic_size//2
    middle_point=[roi[0]+roi[2]//2,roi[1]+roi[3]//2]
    for i in range(2):
        if middle_point[i] < w:
            middle_point[i]  = w
        if middle_point[i] >(512-w):
            middle_point[i]  = 512-w
...
class MyDataset(Dataset):
    ...
    def __init__(self, path,pic_size=256):
        self.img_path = path
    ...
    def __getitem__(self, index):
        ...
        # print(roi)
        image_cut,roi_c = cut_roi(self,image, roi)
        label_image_cut,roi_c = cut_roi(self,label_image, roi)
        # image_cut=label_image_cut
        ...
        # plt.show()
        return image_cut, label_image_cut, roi_c, label_type
